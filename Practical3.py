...
# Taking input of k (size of each group) 
k = int(input())
...
# add that int into list. 
room_list = list(map(int, input().split(" "))) 
for element in room_list:
    # count all the elements in list if count == 1 then printing it and break the loop
    if (room_list.count(element) == 1):
        print(f"{element}")
        break
    # if count != 1 then continuing the loop
    else:
        continue
